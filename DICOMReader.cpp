...
bool readDICOMImage(const std::string& filename, DICOMImage& image) {
    ...
    if (dataset->findAndGetUint16(DCM_BitsAllocated, bitsAllocated).good() &&
        ...
        std::cerr << "Error: Bits Allocated or Samples Per Pixel not found." << std::endl;
        return false;
    }
    ...
    if (dataset->findAndGetSint16(DCM_PixelRepresentation, pixelRepresentation).good()) {
    ...
    size_t numPixels = static_cast<size_t>(image.rows) * static_cast<size_t>(image.cols);
    ...
    if (image.bitsAllocated == 16) {
        ...
        if (dataset->findAndGetUint16Array(DCM_PixelData, pixelData16).good()) {
            ...
            std::cerr << "Error: Unable to retrieve pixel data as Uint16 array." << std::endl;
            return false;
        }
    }
        ...
        if (dataset->findAndGetUint8Array(DCM_PixelData, pixelData8).good()) {
            ...
            std::cerr << "Error: Unable to retrieve pixel data as Uint8 array." << std::endl;
            return false;
        }
    }
    else {
        std::cerr << "Error: Unsupported Bits Allocated: " << image.bitsAllocated << std::endl;
        return false;
    }
